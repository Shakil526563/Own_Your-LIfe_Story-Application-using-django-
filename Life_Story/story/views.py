...
from .forms import BlogPostForm1, ProfileForm, BlogPostForm,BlogPostForm1
...
def hp_story(request):
    postss = storyPost.objects.all()
    postss = storyPost.objects.filter().order_by('-dateTime')
    return render(request, "hp_story.html", {'posts':postss})    
...
def happy_story(request):
    if request.method=="POST":
        form = BlogPostForm1(data=request.POST, files=request.FILES)
        if form.is_valid():
            ...
            obj = form.instance
            alert = True
            return render(request, "happy_story.html",{'obj':obj, 'alert':alert})
    else:
        form=BlogPostForm1()
    return render(request, "happy_story.html", {'form':form})
